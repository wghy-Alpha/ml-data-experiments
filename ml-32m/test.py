...
# 热门物品：交互数排名前5%
top_5_percent = int(len(item_interactions) * 0.001)
hot_items = item_interactions.sort_values('interaction_count', ascending=False).head(top_5_percent)
hot_item_ids = set(hot_items['movieId'])
hot_interactions_sum = hot_items['interaction_count'].sum()

# 长尾物品：从交互数最少的物品开始，累计交互数达到热门物品的总交互数
sorted_items = item_interactions.sort_values('interaction_count', ascending=True)
long_tail_items = []
cumulative = 0
for idx, row in sorted_items.iterrows():
    long_tail_items.append(row['movieId'])
    cumulative += row['interaction_count']
    if cumulative >= hot_interactions_sum:
        break
long_tail_item_ids = set(long_tail_items)
